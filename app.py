...
try:
    ...
    import patch_debug_mode as devpatch
...
class InspectorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(tools, text="Entwicklermodus-Patcher", command=self.open_dev_patch_dialog).pack(side="left", padx=6)
    ...
    def open_dev_patch_dialog(self):

        if devpatch is None:

            messagebox.showerror(

                "Patcher nicht verf�gbar",

                "Das Modul 'patch_debug_mode.py' konnte nicht geladen werden. "

                "Bitte stellen Sie sicher, dass die Datei im gleichen Ordner wie das Programm vorhanden ist.",

            )

            return

        if self._dev_patch_dialog and self._dev_patch_dialog.winfo_exists():

            self._dev_patch_dialog.lift()

            self._dev_patch_dialog.focus_force()
        ...
        win.title("Entwicklermodus-Patcher")
        ...
        instructions = (
            ...
            "Dieses Werkzeug erstellt ein Backup der installierten 'app.asar' und ersetzt es "

            "durch eine gepatchte Variante, die den Entwicklermodus aktiviert. "

            "F�r die Operation werden Node.js (npx asar) sowie Schreibrechte "

            "im Installationsordner ben�tigt. Nach dem Patch kann �ber "

            "'Backup wiederherstellen' jederzeit der Originalzustand zur�ckgespielt werden."

        )

        ttk.Label(win, text=instructions, wraplength=680, justify="left").pack(fill="x", padx=12, pady=(10, 6))
        ...
        detection_msg = tk.StringVar(value="")

        backup_status = tk.StringVar(value="")

        patch_state_var = tk.StringVar(value="")

        force_var = tk.BooleanVar(value=False)

        keep_backup_var = tk.BooleanVar(value=False)



        open_backup_btn = None  # will be set later

        patch_state_label = None
        ...
        def get_current_asar_path() -> Optional[Path]:
        ...
        def refresh_status() -> None:
            ...
            patch_state_color = "#1f3b73"

            backup_button_state = "disabled"
            ...
            if p is None:
                ...
                backup_text = "Bitte eine 'app.asar'-Datei auswhlen."

                backup_color = "#b32424"

            elif not p.exists():

                backup_text = f"Datei nicht gefunden: {p}"

                backup_color = "#b32424"

                patch_state_text = "Status: Datei nicht gefunden."

                patch_state_color = "#b32424"

            else:

                state = devpatch.detect_patch_state(p)

                if state == getattr(devpatch, "PATCH_STATE_PATCHED", "patched"):

                    patch_state_text = "Aktueller Zustand: Entwicklermodus aktiv."

                    patch_state_color = "#1f7f37"

                elif state == getattr(devpatch, "PATCH_STATE_ORIGINAL", "original"):

                    patch_state_text = "Aktueller Zustand: Original (nicht gepatcht)."

                    patch_state_color = "#1f3b73"

                    patch_available = True

                elif state == getattr(devpatch, "PATCH_STATE_UNSUPPORTED", "unsupported"):

                    patch_state_text = "Aktueller Zustand: Unbekannt  Signatur nicht erkannt."

                    patch_state_color = "#b36a00"

                    patch_available = True

                else:

                    patch_state_text = f"Aktueller Zustand: {state}"

                    patch_state_color = "#b36a00"

                    patch_available = True

                backup_file = devpatch.backup_path(p)

                if backup_file.exists():

                    backup_text = f"Backup vorhanden: {backup_file}"

                    backup_button_state = "normal"

                    restore_available = True

                else:

                    backup_text = "Kein Backup gefunden. Patch wurde vermutlich noch nicht angewendet."

                    backup_color = "#b36a00"

            backup_status.set(backup_text)

            if backup_label is not None:

                backup_label.configure(foreground=backup_color)

            patch_state_var.set(patch_state_text)

            if patch_state_label is not None:

                patch_state_label.configure(foreground=patch_state_color)

            if open_backup_btn is not None:

                open_backup_btn.config(state=backup_button_state)

            if not operation_active:

                if patch_btn is not None:

                    patch_btn.config(state="normal" if patch_available else "disabled")

                if restore_btn is not None:

                    restore_btn.config(state="normal" if restore_available else "disabled")
        ...
        def choose_path():
            ...
            if current:
                ...
                initial = os.environ.get("ProgramFiles", "C:\\Program Files")

            file_path = filedialog.askopenfilename(

                title="app.asar ausw�hlen",

                initialdir=initial,

                filetypes=[("ASAR-Dateien", "*.asar"), ("Alle Dateien", "*.*")],
        ...
        def open_backup():
            ...
            if not p:
                ...
                messagebox.showinfo("Backup anzeigen", "Bitte zuerst eine 'app.asar'-Datei ausw�hlen.")
            ...
            if not backup_file.exists():
                ...
                messagebox.showinfo("Backup anzeigen", "Es wurde kein Backup gefunden.")
            ...
            try:
                ...
                messagebox.showerror("Fehler beim �ffnen", f"Das Backup konnte nicht ge�ffnet werden:\n{exc}")
        ...
        def start_patch():
            ...
            if not p:
                ...
                messagebox.showerror("Patch anwenden", "Bitte zuerst eine 'app.asar'-Datei ausw�hlen.")

                return

            if not p.exists():

                messagebox.showerror("Patch anwenden", f"Die Datei wurde nicht gefunden:\n{p}")
            ...
            append_log(f"Starte Patch f�r {p} ...")
            ...
            def worker():
                ...
                try:
                    ...
                    devpatch.apply_patch(

                        p,

                        force_backup=force_var.get(),
                    ...
                    self.after(
                        ...
                        lambda: messagebox.showinfo(
                            ...
                            "Der Patch wurde erfolgreich angewendet.",

                        ),

                    )

                except devpatch.PatchError as exc:
                    ...
                    append_log(f"Berechtigungsfehler: {exc}")
                    ...
                    self.after(
                        ...
                        lambda: messagebox.showerror(
                            ...
                            "Schreibzugriff verweigert. Bitte f�hren Sie das Programm mit Administratorrechten aus.\n"
                    ...
                    def finalize():
                        ...
                        status_msg = patch_state_var.get().strip()
                        if status_msg:
        ...
        def start_restore():
            ...
            if not p:
                ...
                messagebox.showerror("Backup wiederherstellen", "Bitte zuerst eine 'app.asar'-Datei ausw�hlen.")
            ...
            if not backup_file.exists():
                ...
                messagebox.showerror(
                    ...
                    "Es wurde kein Backup gefunden. Bitte zuerst den Patch anwenden.",
            ...
            append_log(f"Starte Wiederherstellung aus {backup_file} ...")
                ...
                try:
                    ...
                    self.after(
                        ...
                        lambda: messagebox.showinfo(
                            ...
                            "Das Original wurde erfolgreich zur�ckgespielt.",
        ...
        path_frame = ttk.LabelFrame(main, text="Pfad zur app.asar")
        ...
        entry.grid(row=0, column=0, sticky="ew", padx=(6, 4), pady=6)

        browse_btn = ttk.Button(path_frame, text="Pfad w�hlen", command=choose_path)

        browse_btn.grid(row=0, column=1, padx=(0, 6), pady=6)



        detect_label = ttk.Label(path_frame, textvariable=detection_msg, foreground="#1f3b73")

        detect_label.grid(row=1, column=0, columnspan=2, sticky="w", padx=6, pady=(0, 4))



        backup_label = ttk.Label(path_frame, textvariable=backup_status, foreground="#1f7f37")
        backup_label.grid(row=2, column=0, columnspan=2, sticky="w", padx=6, pady=(0, 4))

        patch_state_label = tk.Label(path_frame, textvariable=patch_state_var, anchor="w")
        patch_state_label.grid(row=3, column=0, columnspan=2, sticky="w", padx=6, pady=(0, 4))

        open_backup_btn = ttk.Button(path_frame, text="Backup anzeigen", command=open_backup, state="disabled")
        open_backup_btn.grid(row=4, column=0, sticky="w", padx=6, pady=(0, 6))


        options = ttk.LabelFrame(main, text="Optionen")
        ...
        ttk.Checkbutton(
            ...
            text="Bestehendes Backup �berschreiben",
        ...
        ttk.Checkbutton(
            ...
            text="Backup beim Wiederherstellen behalten",
        ...
        patch_btn = ttk.Button(buttons, text="Patch anwenden", command=start_patch)

        patch_btn.pack(side="left")

        restore_btn = ttk.Button(buttons, text="Backup wiederherstellen", command=start_restore)

        restore_btn.pack(side="left", padx=6)

        ttk.Button(buttons, text="Schliessen", command=on_close).pack(side="right")



        log_frame = ttk.LabelFrame(main, text="Protokoll")
        ...
        # Detect default path once UI elements exist.

        try:

            auto_path = devpatch.resolve_asar_path(None)

            path_var.set(str(auto_path))

            detection_msg.set("Installationspfad automatisch erkannt.")

        except devpatch.PatchError as exc:

            detection_msg.set(str(exc))



        def on_path_change(*_args):
        ...
        append_log("Bereit. W�hlen Sie 'Patch anwenden', um den Entwicklermodus zu aktivieren.")
        initial_status = patch_state_var.get().strip()
        if initial_status:
